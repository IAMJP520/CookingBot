#!/usr/bin/env python3
"""
MuJoCo .act 파일을 재생하는 스크립트 (개선된 버전)
사용법: python3 run_act.py [model_xml_path] [act_file_path]
예시: python3 run_act.py dual_arm_robot.xml demo_manual.act
...
def draw_progress_bar(context, viewport, progress, width=200, height=10, x_pos=50, y_pos=50):
    ...
    """
    # 배경 (회색)
    mujoco.mjr_rectangle(viewport, x_pos, y_pos, width, height, [0.3, 0.3, 0.3, 1.0], context)
    
    # 진행 상태 (초록색)
    bar_width = int(width * progress)
    if bar_width > 0:
        mujoco.mjr_rectangle(viewport, x_pos, y_pos, bar_width, height, [0.0, 1.0, 0.0, 1.0], context)
        ...
        while not glfw.window_should_close(window):
            ...
            if show_info:
                ...
                draw_progress_bar(context, viewport, progress, 
                                  width=int(bar_width), height=bar_height, 
                ...
                # 관절 이름 및 값 (최대 10개까지만 표시)
                num_joints_to_show = min(10, num_ctrl)
                
                # 왼쪽 로봇 관절
                left_names = ["좌측 관절1", "좌측 관절2", "좌측 관절3", "좌측 관절4", "좌측 그리퍼"]
                right_names = ["우측 관절1", "우측 관절2", "우측 관절3", "우측 관절4", "우측 그리퍼"]
